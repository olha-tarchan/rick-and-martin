...
const CharacterListComponent = () => {
    ...
    useEffect(() => {
        ...
        if(filterGender !== ''){
            fetch((API_URL + `/?page=${currentPage}&name=rick&gender=${filterGender}`))
                .then(res => res.json())
                .then(data => {
        ...
        if(filterStatus !== ''){
            fetch((API_URL + `/?page=${currentPage}&name=rick&status=${filterStatus}`))
                .then(res => res.json())
                .then(data => {
                    setMaxPages(data.info.pages);
                    return data;
    ...
    }, [currentPage, filterGender, filterStatus]);
    ...
    return (
        <>
            ...
            <div className="characters">
                {data.length > 12
                    ?
                    data.slice(0,-8).map(el => <CharacterItem key={el.id} character={el}/>)
                    :
                    data.map(el => <CharacterItem key={el.id} character={el}/>)
                }
                <div className="pagination">
                    {(currentPage === 1) ?
                        <button disabled className="btn-pagination">Prev</button>
                        :
                        <button onClick={onPrevPage} className="btn-pagination">Prev</button>
                    }
                    {(currentPage === maxPages) ?
                        <button disabled className="btn-pagination">Next</button>
                        :
                        <button onClick={onNextPage} className="btn-pagination">Next</button>
                    }
                </div>
            </div>
        </>
